...
import { Wrench, Mail, Lock, Eye, EyeOff, ArrowRight, User, Shield, UserCog } from "lucide-react";
...
const Login = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center industrial-mesh p-4">
      <main className="w-full max-w-[1200px] grid grid-cols-1 lg:grid-cols-12 bg-card shadow-lg rounded-xl overflow-hidden min-h-[700px]">
        {/* Left Branding */}
        <section className="hidden lg:flex lg:col-span-5 relative flex-col justify-between p-12 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 text-white overflow-hidden">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2 mb-12">
              ...
              <span className="text-2xl font-extrabold tracking-tighter uppercase italic">AutoServe</span>
            </div>
          ...
          <div className="relative z-10">
            <div className="bg-white/5 border border-white/10 rounded-xl p-6 backdrop-blur-md">
              <div className="flex items-center gap-4 mb-4">
                ...
                <span className="text-[10px] uppercase tracking-[0.15em] font-mono text-slate-400">
        ...
        {/* Right Form */}
        <section className="col-span-1 lg:col-span-7 flex flex-col p-8 md:p-16 lg:p-20 justify-center bg-card">
          <div className="mb-10">
            <h2 className="text-2xl font-bold text-on-surface mb-2 tracking-tight">Welcome back</h2>
            <p className="text-muted-foreground text-sm mb-8">Select your access level to continue</p>
            <div className="grid grid-cols-3 gap-3 p-1.5 bg-surface-container rounded-xl border border-outline/20">
              {roles.map((r) => (
                <button
                  key={r.id}
                  onClick={() => setRole(r.id)}
                  className={`flex flex-col items-center gap-2 py-4 px-2 rounded-lg transition-all duration-200 ${
                    role === r.id
                      ? "bg-card shadow-sm border border-outline text-primary"
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-surface-container-low border border-outline/30 rounded-xl py-3.5 pl-11 pr-4 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none placeholder:text-muted-foreground"
                />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••••••"
                  className="w-full bg-surface-container-low border border-outline/30 rounded-xl py-3.5 pl-11 pr-11 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none placeholder:text-muted-foreground"
                />
                <button
                  type="button"
            ...
            <div className="flex items-center gap-3 py-2">
              <input
                type="checkbox"
                id="remember"
                className="w-4 h-4 rounded border-outline/50 text-primary focus:ring-primary/20"
              />
            ...
            <div className="pt-4 space-y-4">
              ...
              <button
                type="button"
                className="w-full bg-surface-container-low border border-outline/20 hover:bg-surface-container-high text-on-surface font-bold py-4 rounded-xl flex items-center justify-center gap-3 transition-all duration-200"
              >
      ...
      {/* Footer */}
      <footer className="fixed bottom-0 left-0 w-full p-6 flex justify-between items-center pointer-events-none">
        <div className="pointer-events-auto flex gap-6">
          <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground">Privacy Protocol</span>
          <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground">Operational Terms</span>
        </div>
        <div className="pointer-events-auto flex items-center gap-4 bg-card/80 backdrop-blur-md px-4 py-2 rounded-full border border-outline shadow-sm">
          <span className="text-[10px] font-mono uppercase tracking-[0.15em] text-muted-foreground">Global Node: East-1</span>
