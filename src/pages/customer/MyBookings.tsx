...
import { Plus, Calendar, MapPin, User, Clock, Wrench, CheckCircle } from "lucide-react";
      ...
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          ...
          <div className="bg-card p-5 rounded-xl border-2 border-primary/20 shadow-sm hover:shadow-md transition-all">
            ...
            {/* Track Status - Micro Interaction */}
            ...
            <tbody className="divide-y divide-border/10">
              {[
                ...
                <tr key={i} className="hover:bg-surface-container-low/50 transition-colors">
                  <td className="py-4 px-6"><div className="flex items-center gap-2"><Wrench className="w-4 h-4 text-muted-foreground" /><span className="text-sm font-semibold text-on-surface">{r.service}</span></div></td>
                  ...
                  <td className="py-4 px-4"><span className={`text-[10px] font-bold px-2 py-0.5 rounded-full flex items-center gap-1 w-fit ${r.status === "Completed" ? "text-emerald-600 bg-emerald-50" : "text-muted-foreground bg-surface-container"}`}>{r.status === "Completed" && <CheckCircle className="w-3 h-3" />}{r.status}</span></td>
                  <td className="py-4 px-4 text-sm font-bold text-on-surface text-right font-mono">{r.total}</td>
                  <td className="py-4 px-4 text-right"><button className="text-xs text-primary font-semibold hover:underline">Invoice</button></td>
                </tr>
              ))}
            </tbody>
          </table>
