...
import { CheckCircle, Settings, Wrench, Zap, CircleDot, Car } from "lucide-react";

const steps = ["Service", "Schedule", "Confirm"] as const;
const services = [
  { name: "Synthetic Oil Change", desc: "Full synthetic oil replacement with 21-point inspection.", price: "$89.99", time: "45 mins", icon: Settings },
  { name: "Tire Rotation & Balance", desc: "Extend tire life and ensure a smooth ride with professional balancing.", price: "$59.00", time: "30 mins", icon: CircleDot },
  { name: "Brake Fluid Flush", desc: "Complete brake fluid replacement to ensure braking safety.", price: "$129.00", time: "60 mins", icon: Wrench },
  { name: "General Diagnostics", desc: "Comprehensive engine scan and manual inspection for any issues.", price: "$149.00", time: "90 mins", icon: Zap },
];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {services.map((s, i) => (
              <button key={s.name} onClick={() => setSelected(i)} className={`text-left bg-card p-5 rounded-xl border-2 transition-all hover:shadow-md group ${selected === i ? "border-primary shadow-md" : "border-border/20 hover:border-border/40"}`}>
                <div className="flex items-center justify-between mb-3">
                  <div className={`p-2 rounded-lg transition-colors ${selected === i ? "bg-primary/10" : "bg-surface-container group-hover:bg-surface-container-high"}`}>
                    <s.icon className={`w-5 h-5 ${selected === i ? "text-primary" : "text-muted-foreground"}`} />
                  </div>
                  {selected === i && <CheckCircle className="w-5 h-5 text-primary" />}
                </div>
          ...
          <button onClick={() => setStep(Math.min(step + 1, 2))} className="mt-6 bg-primary text-primary-foreground px-6 py-3 rounded-lg text-sm font-bold flex items-center gap-2 ml-auto shadow-lg shadow-primary/20 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98] transition-all">
            Next: Schedule →
          </button>
        </div>
        ...
        <div className="space-y-4">
          <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-xl">
            <div className="flex items-center gap-2 mb-3"><Zap className="w-4 h-4 text-primary" /><h4 className="font-bold text-sm">AI Suggested Upsells</h4></div>
            ...
            {[
              ...
              <div key={u.name} className="bg-white/5 border border-white/10 rounded-lg p-3 mb-2">
                ...
                <button className="w-full mt-2 py-1.5 bg-white/10 border border-white/20 rounded text-xs font-bold hover:bg-white/20 active:scale-[0.98] transition-all">Add to Service</button>
              </div>
            ))}
          ...
          <div className="bg-card p-4 rounded-xl border border-border/20 shadow-sm">
            ...
            <div className="flex items-center gap-3">
              ...
              <div><p className="text-sm font-bold text-on-surface">2021 BMW X5</p><p className="text-[10px] font-mono text-muted-foreground">VIN: ...X9420</p></div>
            </div>
          </div>
        </div>
      </div>
