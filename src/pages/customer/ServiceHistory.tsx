...
const records = [
  { service: "Major Interval Service", date: "Jan 14, 2024", miles: "42,500", cost: "$1,245.50", verified: true, diagnosis: "Standard 40k inspection revealed minor wear on rear brake pads and coolant acidity. Engine health stable.", work: ["Synthetic Oil & Filter Exchange", "Rear Brake Pad Replacement", "Coolant System Flush"], parts: ["P/N: 06L-115-562-B", "P/N: 8W0-698-451-R", "G12-EV0-C00L"] },
  { service: "Tire Rotation & Balance", date: "Oct 02, 2023", miles: "38,120", cost: "$85.00", verified: true, diagnosis: "Customer reported slight vibration at 65mph. Found right front tire out of balance by 0.75oz.", work: ["High-speed dynamic balancing", "4-wheel rotation"], parts: ["SERVICE-TRB-01"] },
  { service: "Electrical Diagnostic & Repair", date: "Jul 12, 2023", miles: "35,400", cost: "$412.20", verified: true, diagnosis: "Engine fault code P0301. Found frayed wiring harness connecting to cylinder 1 ignition coil.", work: ["Harness pin extraction and rebuild", "Ignition coil replacement (preventative)"], parts: ["P/N: 06E-905-115-G", "P/N: 4H0-973-702"] },
];

const CustomerServiceHistory = () => (
  <div className="space-y-8">
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      <div><h1 className="text-2xl font-bold text-on-surface tracking-tight">Service Timeline</h1><p className="text-sm text-muted-foreground mt-1">Historical records for VIN: <span className="text-primary font-semibold">JS1AG11B0452</span></p></div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {records.map((r, i) => (
          <div key={i} className="bg-card p-6 rounded-xl border border-border/20 shadow-sm">
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              <div><h4 className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mb-2">Diagnosis</h4><p className="text-sm text-muted-foreground italic leading-relaxed">"{r.diagnosis}"</p></div>
              <div><h4 className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold mb-2">Work Performed</h4><ul className="space-y-1">{r.work.map((w, j) => <li key={j} className="text-sm text-muted-foreground flex items-start gap-2"><span className="text-emerald-600 mt-0.5">✓</span>{w}</li>)}</ul></div>
            </div>
      ...
      <div className="space-y-4">
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white p-5 rounded-xl">
          <div className="flex items-center gap-2 mb-3"><span className="text-amber-400">⚡</span><h4 className="font-bold text-sm">AI Precision Insight</h4></div>
          ...
          {[
            ...
            <div key={tip.name} className="bg-white/5 border border-white/10 rounded-lg p-3 mb-2">
              <div className="flex justify-between items-center"><span className="text-sm font-bold">{tip.name}</span><span className="text-[10px] font-bold bg-white/10 px-2 py-0.5 rounded">{tip.badge}</span></div>
          ...
          <button className="w-full mt-2 py-2.5 bg-primary text-primary-foreground rounded-lg text-sm font-bold">Schedule Selected</button>
        </div>
